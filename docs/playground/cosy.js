// Generated by Haxe 4.1.0-rc.1+41a99a1ce
...
cosy.AstPrinter = class cosy_AstPrinter {
	...
	printStmt(statement) {
		...
		case 2:
			return statement.keyword.lexeme;
		case 3:
			let _g = statement.superclass;
			let declaration = "class " + statement.name.lexeme;
			let declaration1 = _g != null ? " < " + this.printExpr(_g) : "";
			this.isInClass = true;
			let body = this.printBlock(statement.methods);
			this.isInClass = false;
		...
		case 5:
			...
			return "for " + (_g1 != null ? _g1.lexeme + " in " : "") + this.printExpr(statement.from) + ".." + this.printExpr(statement.to) + " " + this.printBlock(statement.body);
		case 6:
			return "for " + statement.name.lexeme + " in " + this.printExpr(statement.array) + " " + this.printBlock(statement.body);
		case 7:
			let _g2 = statement.cond;
			return "for " + (_g2 != null ? this.printExpr(_g2) : "") + " " + this.printBlock(statement.body);
		case 8:
			let _g3 = statement.foreign;
			let _g4 = statement.body;
			let _g5 = statement.params;
			let declaration2 = "" + (this.isInClass ? "" : _g3 ? "foreign fn" : "fn") + " " + statement.name.lexeme;
			let _g6 = [];
			let _g7 = 0;
			while(_g7 < _g5.length) _g6.push(this.formatParam(_g5[_g7++]));
			let parameters = _g6.join(", ");
			if(_g3) {
				return "" + declaration2 + "(" + parameters + ")";
			}
			return "" + declaration2 + "(" + parameters + ") " + this.printBlock(_g4);
		case 9:
			let _g8 = statement.el;
			return "if " + this.printExpr(statement.cond) + " " + this.printStmt(statement.then) + (_g8 != null ? " else " + this.printStmt(_g8) : "");
		case 10:
			return "" + statement.keyword.lexeme + " " + this.printExpr(statement.e);
		case 11:
			let _g9 = statement.value;
			return statement.keyword.lexeme + (_g9 != null ? " " + this.printExpr(_g9) : "");
		case 12:
			return "struct " + statement.name.lexeme + " " + this.printBlock(statement.declarations);
		case 13:
			let _g10 = statement.init;
			return "" + (statement.foreign ? "foreign " : "") + (statement.mut ? "mut" : "var") + " " + statement.name.lexeme + (_g10 != null ? " = " + this.printExpr(_g10) : "");
		}
	}
	printExpr(expr) {
...
cosy.Interpreter = class cosy_Interpreter {
	...
	execute(statement) {
		while(true) {
			...
			case 3:
				let _g = statement.methods;
				let _g1 = statement.superclass;
				let _g2 = statement.name;
				let superclass;
				if(_g1 != null) {
					let sc = this.evaluate(_g1);
					if(!((sc) instanceof cosy.Klass)) {
						let this1;
						if(_g1._hx_index == 14) {
							this1 = _g1.name;
						} else {
							throw haxe.Exception.thrown("unreachable");
						}
						throw haxe.Exception.thrown(new cosy.RuntimeError(this1,"Superclass must be a class"));
					}
					superclass = sc;
				} else {
					superclass = null;
				}
				this.environment.define(_g2.lexeme,null);
				if(superclass != null) {
					this.environment = new cosy.Environment(this.environment);
					this.environment.define("super",superclass);
				}
				let methods = new haxe.ds.StringMap();
				let _g3 = 0;
				while(_g3 < _g.length) {
					let method = _g[_g3];
					++_g3;
					if(method._hx_index == 8) {
						let _g = method.name;
						methods.h[_g.lexeme] = new cosy.Function(_g,method.params,method.body,this.environment,_g.lexeme == "init");
					}
				}
				let klass = new cosy.Klass(_g2.lexeme,superclass,methods);
				if(superclass != null) {
					this.environment = this.environment.enclosing;
				}
				this.environment.assign(_g2,klass);
				break;
			case 4:
			...
			case 5:
				...
				try {
					...
					while(_g < _g1) {
						let counter = _g++;
						if(_g5 != null) {
							env.define(_g5.lexeme,counter);
						}
						try {
							this.executeBlock(_g4,env);
						} catch( _g1 ) {
					...
					if(!((haxe.Exception.caught(_g7).unwrap()) instanceof cosy.Break)) {
						throw _g7;
					}
				}
				break;
			case 6:
				let _g8 = statement.body;
				let _g9 = statement.name;
				let arr = this.evaluate(statement.array);
				let env1 = new cosy.Environment(this.environment);
				try {
					let _g = 0;
					while(_g < arr.length) {
						env1.define(_g9.lexeme,arr[_g++]);
						try {
							this.executeBlock(_g8,env1);
						} catch( _g1 ) {
							if(!((haxe.Exception.caught(_g1).unwrap()) instanceof cosy.Continue)) {
								throw _g1;
							}
					...
					if(!((haxe.Exception.caught(_g10).unwrap()) instanceof cosy.Break)) {
						throw _g10;
					}
				}
				break;
			case 7:
				let _g11 = statement.body;
				let _g12 = statement.cond;
				let env2 = new cosy.Environment(this.environment);
				try {
					while(_g12 != null ? this.isTruthy(this.evaluate(_g12)) : true) try {
						this.executeBlock(_g11,env2);
					} catch( _g ) {
					...
					if(!((haxe.Exception.caught(_g13).unwrap()) instanceof cosy.Break)) {
						throw _g13;
					}
				}
				break;
			case 8:
				...
				if(statement.foreign) {
					this.environment.define(_g14.lexeme,cosy.Cosy.foreignFunctions.h[_g14.lexeme]);
					return;
				}
				this.environment.define(_g14.lexeme,new cosy.Function(_g14,statement.params,statement.body,this.environment,false));
				break;
			case 9:
				let _g15 = statement.el;
			...
			case 10:
				...
				break;
			case 11:
				let _g16 = statement.value;
				throw haxe.Exception.thrown(new cosy.Return(_g16 == null ? null : this.evaluate(_g16)));
			case 12:
				let _g17 = statement.declarations;
				let _g18 = statement.name;
				...
				this.environment.assign(_g18,new cosy.StructInstance(_g18,fields));
				break;
			case 13:
				let _g20 = statement.init;
				let _g21 = statement.name;
				if(statement.foreign) {
					this.environment.define(_g21.lexeme,cosy.Cosy.foreignVariables.h[_g21.lexeme]);
					return;
				}
				let value = cosy.Interpreter.uninitialized;
				if(_g20 != null) {
					value = this.evaluate(_g20);
				}
				if(((value) instanceof cosy.StructInstance)) {
					value = value.clone();
				}
				this.environment.define(_g21.lexeme,value);
				break;
			}
			return;
		}
	...
	evaluate(expr) {
		...
		case 11:
			let _g15 = expr.method;
			let distance = this.locals.h[expr.__id__];
			let superclass = this.environment.getAt(distance,"super");
			let obj2 = this.environment.getAt(distance - 1,"this");
			let method = superclass.findMethod(_g15.lexeme);
			if(method == null) {
				throw haxe.Exception.thrown(new cosy.RuntimeError(_g15,"Undefined property \"" + _g15.lexeme + "\"."));
			}
			return method.bind(obj2);
		case 12:
			let _g16 = expr.decls;
			let _g17 = expr.name;
			let structObj = this.lookUpVariable(_g17,expr);
			if(!((structObj) instanceof cosy.StructInstance)) {
				throw haxe.Exception.thrown(new cosy.RuntimeError(_g17,"Struct initializer on non-struct object."));
			}
		...
		case 13:
			...
			case 10:
				this.checkNumberOperand(_g19,right5);
				return -right5;
			case 18:
...
cosy.Function = class cosy_Function {
	...
	bind(instance) {
		let env = new cosy.Environment(this.closure);
...
cosy.JavaScriptPrinter = class cosy_JavaScriptPrinter {
	...
	printStmt(statement) {
		...
		case 3:
			let _g = statement.superclass;
			let className = statement.name.lexeme;
			this.classNames.push(className);
			let declaration = _g != null ? " extends " + this.printExpr(_g) : "";
			this.isInClass = true;
			let body = this.printBlock(statement.methods);
			this.isInClass = false;
		...
		case 5:
			let _g1 = statement.name;
			let counter = _g1 != null ? _g1.lexeme : "__i";
		...
		case 7:
			let _g2 = statement.cond;
			return "while (" + (_g2 != null ? this.printExpr(_g2) : "true") + ") " + this.printBlock(statement.body);
		case 8:
			...
			let declaration1 = "" + (this.isInClass ? "" : "function ") + statement.name.lexeme;
			let _g5 = [];
			let _g6 = 0;
			while(_g6 < _g4.length) _g5.push(_g4[_g6++].name.lexeme);
			return "" + declaration1 + "(" + _g5.join(",") + ") " + this.printStmt(cosy.Stmt.Block(_g3));
		case 9:
		...
		case 11:
			...
			return "return" + (_g8 != null ? " " + this.printExpr(_g8) : "") + ";";
		case 12:
		...
		case 13:
			...
			return "" + (statement.mut ? "var" : "const") + " " + statement.name.lexeme + (_g9 != null ? " = " + this.printExpr(_g9) : "") + ";";
		}
	}
	printExpr(expr) {
		switch(expr._hx_index) {
...
cosy.Klass = class cosy_Klass {
	...
	call(interpreter,args) {
		let instance = new cosy.Instance(this);
		let _g = this.findMethod("init");
		if(_g != null) {
	...
	findMethod(name) {
		if(Object.prototype.hasOwnProperty.call(this.methods.h,name)) {
			return this.methods.h[name];
		}
		...
		while(true) {
			...
			case 3:
				...
				if(_g1 != null) {
					...
					this.resolveExpr(_g1);
					this.beginScope();
					let this1 = this.scopes;
					this1[this1.length - 1].h["super"] = { name : new cosy.Token(cosy.TokenType.Super,"super",null,_g2.line), state : cosy._Resolver.VariableState.Read, mutable : false, member : false};
				}
				this.beginScope();
				let this1 = this.scopes;
				this1[this1.length - 1].h["this"] = { name : new cosy.Token(cosy.TokenType.This,"this",null,_g2.line), state : cosy._Resolver.VariableState.Read, mutable : false, member : false};
				let _g3 = 0;
				while(_g3 < _g.length) {
					let method = _g[_g3];
					++_g3;
					if(method._hx_index == 8) {
						let _g = method.name;
						this.resolveFunction(_g,method.params,method.body,_g.lexeme == "init" ? cosy._Resolver.FunctionType.Initializer : cosy._Resolver.FunctionType.Method,false);
					}
				}
				this.endScope();
				if(_g1 != null) {
			...
			case 6:
				...
				this.define(_g7);
				if(_g6.length == 0) {
					cosy.Cosy.error(cosy.ErrorDataType.Token(_g7),"Loop body is empty.");
				}
				this.resolveStmts(_g6);
				this.endScope();
				break;
			case 7:
				let _g8 = stmt.cond;
				...
				break;
			case 8:
				let _g9 = stmt.foreign;
				let _g10 = stmt.name;
				if(_g9 && !Object.prototype.hasOwnProperty.call(cosy.Cosy.foreignFunctions.h,_g10.lexeme)) {
					cosy.Cosy.error(cosy.ErrorDataType.Token(_g10),"Foreign function not set.");
				}
				this.declare(_g10);
				this.define(_g10);
				this.resolveFunction(_g10,stmt.params,stmt.body,cosy._Resolver.FunctionType.Function,_g9);
				break;
			case 9:
				let _g11 = stmt.el;
			...
			case 11:
				let _g12 = stmt.value;
				let _g13 = stmt.keyword;
				if(this.currentFunction == cosy._Resolver.FunctionType.None) {
					cosy.Cosy.error(cosy.ErrorDataType.Token(_g13),"Cannot return from top-level code.");
				}
				if(_g12 != null) {
					if(this.currentFunction == cosy._Resolver.FunctionType.Initializer) {
						cosy.Cosy.error(cosy.ErrorDataType.Token(_g13),"Cannot return value from an initializer.");
					}
			...
			case 13:
				...
				let _g16 = stmt.init;
				let _g17 = stmt.name;
				if(stmt.foreign && !Object.prototype.hasOwnProperty.call(cosy.Cosy.foreignVariables.h,_g17.lexeme)) {
					cosy.Cosy.error(cosy.ErrorDataType.Token(_g17),"Foreign variable not set.");
				}
				let member = this.currentStruct._hx_index == 1;
				this.declare(_g17,_g15,member);
				if(_g16 != null) {
					this.resolveExpr(_g16);
				}
				this.define(_g17,_g15,member);
				break;
			}
			return;
		}
		...
		while(true) {
			...
			case 9:
				...
				case 14:
					...
					break;
				default:
					console.log("src/cosy/Resolver.hx:183:",_g6);
					throw haxe.Exception.thrown("this is unexpected");
				}
				break;
			case 10:
				let _g7 = expr.keyword;
				if(this.currentClass == cosy._Resolver.ClassType.None) {
					cosy.Cosy.error(cosy.ErrorDataType.Token(_g7),"Cannot use \"this\" outside of a class.");
				} else {
					this.resolveLocal(expr,_g7,true);
				}
				break;
			case 11:
				let _g8 = expr.keyword;
				switch(this.currentClass._hx_index) {
				case 0:
					cosy.Cosy.error(cosy.ErrorDataType.Token(_g8),"Cannot use \"super\" outside of a class.");
					break;
				case 1:
					cosy.Cosy.error(cosy.ErrorDataType.Token(_g8),"Cannot use \"super\" in a class with no superclass.");
					break;
				case 2:
					break;
				}
			...
			case 14:
				...
				let tmp;
				let this1 = this.scopes;
				if(Object.prototype.hasOwnProperty.call(this1[this1.length - 1].h,_g12.lexeme)) {
					let this1 = this.scopes;
					tmp = this1[this1.length - 1].h[_g12.lexeme].state._hx_index == 0;
				} else {
					tmp = false;
				}
				if(tmp) {
					cosy.Cosy.error(cosy.ErrorDataType.Token(_g12),"Cannot read local variable in its own initializer");
				}
				if(StringTools.startsWith(_g12.lexeme,"_")) {
					cosy.Cosy.error(cosy.ErrorDataType.Token(_g12),"Variables starting with _ are considered unused.");
				}
				this.resolveLocal(expr,_g12,true);
				break;
			case 15:
...
cosy.Scanner.keywords = (function($this) {
	...
	_g.h["class"] = cosy.TokenType.Class;
	...
	_g.h["super"] = cosy.TokenType.Super;
	_g.h["this"] = cosy.TokenType.This;
